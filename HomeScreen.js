...
const HomeScreen = () => {
  ...
  const handleUpdateActivity = useCallback(
    (updatedActivity) => {
      console.log("Actualizando actividad..."); // Mensaje breve

      setActivities((prev) => {
        if (typeof prev === "function" || !Array.isArray(prev)) {
          console.error("Error: Estado de actividades inválido.");
          return prev;
        }
        ...
        if (!Array.isArray(newActivities)) {
          console.error("Error al actualizar actividades.");
          return prev;
        }

        console.log("Actividad actualizada correctamente."); // Mensaje final
